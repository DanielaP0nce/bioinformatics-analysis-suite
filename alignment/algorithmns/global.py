...
def build_scoring_matrix(file):
    ...
    :param file: The path to the file containing the scoring matrix.
    :type file: str
    :raises FileNotFoundError: If the file does not exist.
    :raises IndexError: If the file is not in the correct format.
    :returns: A dictionary representing the scoring matrix. The keys are the nucleotides, and the values are dictionaries containing the scores for each nucleotide.
    :rtype: dict
    """
    scoring_dict = {}
    try:
...
def initialize_alignment_table(file, gap_penalty):
    ...
    Raises:
        ...
        IndexError: If the file does not contain two lines.
    """
    try:
        with open(file, "r") as text:
            text = text.readlines()
